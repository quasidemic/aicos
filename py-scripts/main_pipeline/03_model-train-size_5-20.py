...
## LOGGING SETUP
logging.basicConfig(
    filename=join(logs_dir, 'eval_train_size_binary_sep25-1.log'),  # Log file name
...
def fit_model(n, seed, labels, model_name = model_name, params = params, device = device, test_data = test_data, logger = logger):
    ...
    # Write to file
    out_path = join(output_dir, "n-art_binary_model-eval_class-reps-1_revision-sep25.json")
...
## FITTING MODELS
train_sizes = list(range(5,25,5)) # train size set as number of articles
